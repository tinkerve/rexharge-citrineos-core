...
You will notice in `/ocpp-server/everest` directory the files created to support running EVerest within Docker.
...
the `/ocpp-server/everest` directory so that it can pick up the `Dockerfile` and the `docker-compose.yml` files.
